...
#include "assets/objects/object_link_deity/object_link_deity.h"
#include "assets/objects/object_mask_deity/object_mask_deity.h"
...
FlexSkeletonHeader* gPlayerSkelHeaders[] = { &gLinkAdultSkel, &gLinkChildSkel, &gLinkFierceDeitySkel }; // Custom Forms: skeleton headers
...
u8 sActionModelGroups[PLAYER_IA_MAX] = {
    ...
    PLAYER_MODELGROUP_DEFAULT,       // PLAYER_IA_MASK_DEITY
};
...
Gfx* sPlayerRightHandShieldDLs[PLAYER_SHIELD_MAX * (NUM_FORMS * 2)] = {
    // PLAYER_SHIELD_NONE
...
Gfx* D_80125D28[(PLAYER_SHIELD_MAX + 2) * (NUM_FORMS * 2)] = {
    // PLAYER_SHIELD_NONE
...
Gfx* D_80125D88[(PLAYER_SHIELD_MAX + 2) * (NUM_FORMS * 2)] = {
    // PLAYER_SHIELD_NONE
...
Gfx* gPlayerLeftHandBgsDLs[] = {
    ...
    NULL,
    // fierce deity sword
    gLinkAdultFierceDeityLeftHandHoldingSwordDL,
    gLinkChildFierceDeityLeftHandHoldingSwordDL,
    gLinkFierceDeityLeftHandHoldingSwordDL,
    gLinkAdultFierceDeityLeftHandHoldingSwordDL,
    gLinkChildFierceDeityLeftHandHoldingSwordDL,
    gLinkFierceDeityLeftHandHoldingSwordDL,
...
Gfx* sPlayerLeftHandBottleDLs[] = {
    ...
    gLinkFierceDeityLeftHandDL, // gLinkFierceDeityLeftHandHoldBottleDL was unused
    gLinkAdultLeftHandOutNearDL,
    gLinkChildLeftHandUpNearDL,
    gLinkFierceDeityLeftHandDL, // gLinkFierceDeityLeftHandHoldBottleDL was unused
};
...
static AnimatedMatTexScrollParams sMaskEffectScroll[] = {
    { -1, 0, 0x10, 0x10 },
    { 1, -2, 0x10, 0x10 },
};

static Vec2f D_801C0E04[LINK_AGE_MAX] = {
    { 0.0f, 0.0f },      // Adult
    { 0.0f, 0.0f },      // Child
    { 140.0f, -130.0f }, // Deity
};

static Gfx gEmptyMaskDL[] = {
    gsSPEndDisplayList(),
};

static Gfx* sTransformMaskDLists[] = { // regular mask
    &gEmptyMaskDL, // adult (shouldn't be possible)
    &gEmptyMaskDL, // child (shouldn't be possible)
    gFierceDeityMaskDL, // deity
};
...
static MaskObjectDL sTransformObjectMaskDLists[] = { // Link is screaming, this is at the end of the sequence
    {OBJECT_INVALID, &gEmptyMaskDL}, // adult (shouldn't be possible)
    {OBJECT_INVALID, &gEmptyMaskDL}, // child (shouldn't be possible)
    {OBJECT_MASK_DEITY, object_mask_deity_DL_000900}, // deity
};

u8 Player_RequestMaskObject(Player* this, s16 objectId) { // overengineered even though you could have a loading flag get set before this is needed and it will always be set until it's done hehehoohoo
    s32 pad;
    u32 size;
    ...
    if (this->maskObjectLoading == 0 && !(this->maskObjectId == objectId && this->maskObjectLoading != 0)) { // not invalid or this object is already loading/loaded
        this->maskObjectLoading = 1;
        this->maskObjectId = objectId;
        osCreateMesgQueue(&this->maskObjectLoadQueue, &this->maskObjectLoadMsg, 1);

        size = gObjectTable[objectId].vromEnd - gObjectTable[objectId].vromStart;

        LOG_HEX("size", size, "../z_player.c", __LINE__);
        ASSERT(size <= 0x3800, "maskObject <- size <= 0x3800", "../z_player.c", __LINE__);

        DmaMgr_RequestAsync(&this->maskObjectDmaRequest, this->maskObjectSegment, gObjectTable[objectId].vromStart, size, 0,
                            &this->maskObjectLoadQueue, NULL, "../z_player.c", __LINE__);
    } else if (objectId == this->maskObjectId && this->maskObjectLoading == 1) { // this object is loading right now
        if (osRecvMesg(&this->maskObjectLoadQueue, NULL, OS_MESG_NOBLOCK) == 0) { // successfully loaded
            this->maskObjectLoading = 2;
            return true;
        }
    } else if (objectId == this->maskObjectId && this->maskObjectLoading == 2) {
        return true;
    }
...
void Player_DrawMaskInHand(PlayState* play, Player* this) {
    s32 temp_v1 = this->skelAnime.animation == &gPlayerAnim_cl_maskoff;
    f32 temp_f0;

    if (temp_v1 ||
        ((LINK_IS_HUMAN) && (this->skelAnime.animation == &gPlayerAnim_cl_setmask) &&
         (temp_f0 = this->skelAnime.curFrame - 8.0f, (temp_f0 >= 0.0f)) && (temp_f0 < 4.0f))/* ||
        (player->stateFlags2 & PLAYER_STATE2_1000000)*/) {
        s32 mask;

        if (temp_v1) {
            mask = this->transformPreviousForm;
        } else {
            mask = this->transformTargetForm;
        }

        if (/*func_80127438(play, player, mask)*/1) {
            OPEN_DISPS(play->state.gfxCtx, "..z_player_lib", __LINE__);

            Matrix_Push();
            Matrix_Translate(-323.67f, 412.15f, -969.96f, MTXMODE_APPLY);
            MatrixMM_RotateZYX(-0x32BE, -0x50DE, -0x7717, MTXMODE_APPLY);

            gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(play->state.gfxCtx, "../z_player_lib.c", __LINE__),
                  G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);

            gSPDisplayList(POLY_OPA_DISP++, sTransformMaskDLists[mask]);

            Matrix_Pop();

            CLOSE_DISPS(play->state.gfxCtx, "..z_player_lib", __LINE__);
        }

    }
}

s16 sDeityBootData[18] = {
    /* 0  */ 200,  // REG(19)
    /* 1  */ 666,  // REG(30)
    /* 2  */ 200,  // REG(32)
    /* 3  */ 700,  // REG(34)
...
void Player_SetBootData(PlayState* play, Player* this) {
    ...
    if (LINK_IS_DEITY) {
        ...
        //currentBoots = PLAYER_BOOTS_KOKIRI_FIERCE_DEITY;
        bootRegs = sDeityBootData;
        REG(19) = bootRegs[0];
        REG(30) = bootRegs[1];
        REG(32) = bootRegs[2];
        REG(34) = bootRegs[3];
...
s32 Player_IsChildWithHylianShield(Player* this) {
    return gSaveContext.linkAge == LINK_AGE_CHILD && (this->currentShield == PLAYER_SHIELD_HYLIAN);
}
...
s32 Player_HoldsBrokenKnife(Player* this) {
    if (this->heldItemId == ITEM_SWORD_DEITY) {
        return false;
    }
...
void* sEyeTextures[NUM_FORMS][8] = { // Custom Forms: eyes
    { gLinkAdultEyesOpenTex, gLinkAdultEyesHalfTex, gLinkAdultEyesClosedfTex, gLinkAdultEyesRollLeftTex,
      gLinkAdultEyesRollRightTex, gLinkAdultEyesShockTex, gLinkAdultEyesUnk1Tex, gLinkAdultEyesUnk2Tex },
...
void* sMouthTextures[NUM_FORMS][4] = { // Custom Forms: mouth
...
void Player_DrawImpl(PlayState* play, void** skeleton, Vec3s* jointTable, s32 dListCount, s32 lod, s32 tunic, s32 boots,
    ...
    switch (tunic) {
        case PLAYER_TUNIC_KOKIRI:
            gDPSetEnvColor(POLY_OPA_DISP++, gSaveContext.kokiriTunic[0], gSaveContext.kokiriTunic[1], gSaveContext.kokiriTunic[2], 0);
            break;
        case PLAYER_TUNIC_GORON:
            gDPSetEnvColor(POLY_OPA_DISP++, gSaveContext.goronTunic[0], gSaveContext.goronTunic[1], gSaveContext.goronTunic[2], 0);
            break;
        case PLAYER_TUNIC_ZORA:
            gDPSetEnvColor(POLY_OPA_DISP++, gSaveContext.zoraTunic[0], gSaveContext.zoraTunic[1], gSaveContext.zoraTunic[2], 0);
            break;
        default:
            gDPSetEnvColor(POLY_OPA_DISP++, 0x255, 0x255, 0x255, 0);
            break;
    }

    sDListsLodOffset = lod * NUM_FORMS;
    ...
    if ((overrideLimbDraw != Player_OverrideLimbDrawGameplayFirstPerson) &&
        ...
        if (!LINK_IS_DEITY) { // Custom Forms: Strength Upgrades
            if (LINK_IS_ADULT) {
                s32 strengthUpgrade = CUR_UPG_VALUE(UPG_STRENGTH);
...
// Custom Forms: Miscellaneous

Vec3f D_80126038[NUM_FORMS] = {
    { 1304.0f, 0.0f, 0.0f },
    { 695.0f, 0.0f, 0.0f },
    { 408.0, 0.0f, 0.0f },
};

f32 D_80126050[NUM_FORMS] = { 1265.0f, 826.0f, 359.0, };
f32 D_80126058[NUM_FORMS] = { SQ(13.04f), SQ(6.95f), 16.6463985443f, };
f32 D_80126060[NUM_FORMS] = { 10.019104f, -19.925102f, 3.7582988739f, };
f32 D_80126068[NUM_FORMS] = { 5.0f, 3.0f, 1.0f, };
...
s32 Player_OverrideLimbDrawGameplayCommon(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot,
    ...
    if (limbIndex == PLAYER_LIMB_ROOT) {
        ...
        if (!LINK_IS_ADULT && !LINK_IS_DEITY) { // Custom Forms: root mod
        ...
        if (limbIndex == PLAYER_LIMB_L_HAND) {
            ...
            *dList = *(dLists + sDListsLodOffset);
            ...
            if (sRightHandType == PLAYER_MODELTYPE_RH_SHIELD) {
                dLists += this->currentShield * (NUM_FORMS * 2);
            ...
            *dList = *(dLists + sDListsLodOffset);
            ...
            if ((this->sheathType == PLAYER_MODELTYPE_SHEATH_18) || (this->sheathType == PLAYER_MODELTYPE_SHEATH_19)) {
                ...
                if (!LINK_IS_ADULT && (this->currentShield < PLAYER_SHIELD_HYLIAN) &&
                    ...
                    dLists += PLAYER_SHIELD_MAX * (NUM_FORMS * 2);
                }
            } else if (!LINK_IS_ADULT &&
                       ((this->sheathType == PLAYER_MODELTYPE_SHEATH_16) ||
                        (this->sheathType == PLAYER_MODELTYPE_SHEATH_17)) &&
                       (gSaveContext.equips.buttonItems[0] != ITEM_SWORD_KOKIRI)) {
                dLists = D_80125D28 + PLAYER_SHIELD_MAX * (NUM_FORMS * 2);
            }
...
Gfx* sBottleDLists[NUM_FORMS] = { gLinkAdultBottleDL, gLinkChildBottleDL, gLinkFierceDeityBottleDL };
...
void Player_PostLimbDrawGameplay(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx) {
    ...
    if (limbIndex == PLAYER_LIMB_L_HAND) {
        ...
        if ((Player_CanUseSwordBeams(this) == 1) && this->meleeWeaponState == 0) {
            ...
            Matrix_MultVec3f(&D_801260A4[1], &newBasePos[1]);
            Matrix_MultVec3f(&D_801260A4[2], &newBasePos[2]);
            func_80090480(play, NULL, &this->meleeWeaponInfo[0], &sp124[0], &newBasePos[0]);
        }
        ...
        if (limbIndex == PLAYER_LIMB_SHEATH) {
            ...
            if (this->stateFlags3 & PLAYER_STATE3_TRANSFORMATION_MASK) {
                // draw the blue mask effect over Link's face
                if ((this->transformEventTimer2 != 0)) {               // (player->stateFlags1 & (PLAYER_STATE1_2))
                    static Vec3f D_801C0E40[LINK_AGE_MAX] = {
                        { -230.0f, -520.0f, 0.0f },  // adult
                        { -230.0f, -520.0f, 0.0f },  // child
                        { 0.0f, 0.0f, 0.0f },        // deity
                    };
                    Vec3f* temp_s0_7 = &D_801C0E40[gSaveContext.linkAge];

                    OPEN_DISPS(play->state.gfxCtx, "../z_player_lib.c", __LINE__);

                    Matrix_Push();
                    AnimatedMat_DrawTwoTexScroll(play, 0x0B, &sMaskEffectScroll);
                    Matrix_Translate(temp_s0_7->x, temp_s0_7->y, 0.0f, MTXMODE_APPLY);
                    gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(play->state.gfxCtx, "../z_player_lib.c", __LINE__),
                        G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
                    gDPSetEnvColor(POLY_XLU_DISP++, 0, 0, 255, (u8)this->transformEventTimer2);
                    gSPDisplayList(POLY_XLU_DISP++, gTransformEffectDL);

                    Matrix_Pop();

                    CLOSE_DISPS(play->state.gfxCtx, "../z_player_lib.c", __LINE__);
                }

                // draw a mask on Link's face.
                if (LINK_IS_HUMAN && ((this->skelAnime.animation != &gPlayerAnim_cl_setmask) || (this->skelAnime.curFrame >= 12.0f))) {
                    Vec2f* temp_s0_4 = &D_801C0E04[gSaveContext.linkAge]; // a scale
                    Gfx* displayList;
                    OPEN_DISPS(play->state.gfxCtx, "../z_player_lib.c", 2633);

                    Matrix_Push();
                    Matrix_Translate(temp_s0_4->x, temp_s0_4->y, 0.0f, MTXMODE_APPLY);
                    Matrix_Scale(1.0f, 1.0f - this->transformMatrixModifiers[3], 1.0f - this->transformMatrixModifiers[2], MTXMODE_APPLY);

                    gSPMatrix(OVERLAY_DISP++, Matrix_NewMtx(play->state.gfxCtx, "../z_player_lib.c", __LINE__),
                        G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);

                    Matrix_Pop();

                    if (((this->skelAnime.animation == &gPlayerAnim_cl_setmask) && (this->skelAnime.curFrame >= 51.0f)) || (this->skelAnime.animation == &gPlayerAnim_cl_setmaskend)) {
                        // A dynamically loaded mask object should be used
                        MaskObjectDL maskObject = sTransformObjectMaskDLists[this->transformTargetForm];
                        if (maskObject.objectId == OBJECT_INVALID) { // this is not dynamically loaded
                            gSPDisplayList(POLY_OPA_DISP++, maskObject.displayList);
                        } else if (Player_RequestMaskObject(this, maskObject.objectId)) { // draw it if the object is loaded into the segment
                            gSPSegment(POLY_OPA_DISP++, 0xA, this->maskObjectSegment);
                            gSPDisplayList(POLY_OPA_DISP++, maskObject.displayList);
                        }
                    } else { // a regular mask should be used
                        gSPDisplayList(POLY_OPA_DISP++, sTransformMaskDLists[this->transformTargetForm]);
                    }

                    CLOSE_DISPS(play->state.gfxCtx, "../z_player_lib.c", 2633);
                }
            }
        } else {
...
s32 Player_OverrideLimbDrawPause(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* arg) {
    ...
    if (limbIndex == PLAYER_LIMB_L_HAND) {
        ...
        sLeftHandType = type;
        if ((type == PLAYER_MODELTYPE_LH_BGS) && ((gSaveContext.swordHealth <= 0.0f && !LINK_AGE_DEITY))) {
            dListOffset = (NUM_FORMS * 2);
        }
    } else if (limbIndex == PLAYER_LIMB_R_HAND) {
        type = gPlayerModelTypes[modelGroup][PLAYER_MODELGROUPENTRY_RIGHT_HAND];
        sRightHandType = type;
        if (type == PLAYER_MODELTYPE_RH_SHIELD) {
            dListOffset = playerSwordAndShield[1] * (NUM_FORMS * 2);
        }
    } else if (limbIndex == PLAYER_LIMB_SHEATH) {
