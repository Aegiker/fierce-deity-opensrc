...
void Scene_DrawConfigKokiriForest(PlayState* play) {
    ...
    } else if ((!IS_CUTSCENE_LAYER || ERA_IN_FUTURE) && GET_EVENTCHKINF(EVENTCHKINF_07)) {
        spA0 = 2150;
    }
...
void Scene_DrawConfigLakeHylia(PlayState* play) {
    ...
    if (IS_CUTSCENE_LAYER || (ERA_IN_FUTURE && !GET_EVENTCHKINF(EVENTCHKINF_69))) {
...
f32 D_8012A398 = 0.0f;
#define JABU_SCROLL_SEG1 0x08
#define JABU_SCROLL_SEG2 0x0B
#define JABU_MTX_SEG 0x0D
void Scene_DrawConfigJabuJabu(PlayState* play) {
    ...
    gSPSegment(POLY_OPA_DISP++, JABU_MTX_SEG, Matrix_NewMtx(play->state.gfxCtx, "../z_scene_table.c", 7809));
