...
void Play_Init(GameState* thisx) {
    ...
    SystemArena_Display();
    GameState_Realloc(&this->state, 0x2D4790); // expand RAM; 0x1D4790
    KaleidoManager_Init(this);
    View_Init(&this->view, gfxCtx);
    Audio_SetExtraFilter(0);
    Quake_Init();

    this->darkLinkAmbushFlag = false;
    this->screenshotTimer = 0;
    if (this->screenshotState == 0xAB) {
        this->screenshotState = TESTSTATE_NONE;
    } //
    gSaveContext.lostItemCollectionFlag = 0;
    ...
    if ((gEntranceTable[((void)0, gSaveContext.entranceIndex)].sceneId == SCENE_HYRULE_FIELD) && ERA_IN_PAST &&
        !IS_CUTSCENE_LAYER) {
        if (CHECK_QUEST_ITEM(QUEST_KOKIRI_EMERALD) && CHECK_QUEST_ITEM(QUEST_GORON_RUBY) &&
            ...
            gSaveContext.sceneLayer = 0;
               ...
               ERA_IN_FUTURE && !IS_CUTSCENE_LAYER) {
    ...
    if (gSaveContext.nextDayTime != NEXT_TIME_NONE) {
        if (gSaveContext.nextDayTime == NEXT_TIME_DAY) {
            ...
            if (Inventory_ReplaceItem(this, ITEM_WEIRD_EGG, ITEM_CHICKEN)) { // child egg
                Message_StartTextbox(this, 0x3066, NULL);
                if (gSaveContext.recoveryItems[SLOT_TRADE_CHILD] == ITEM_WEIRD_EGG) gSaveContext.recoveryItems[SLOT_TRADE_CHILD] = ITEM_WEIRD_EGG;
            }

            if (Inventory_ReplaceItem(this, ITEM_POCKET_EGG, ITEM_POCKET_CUCCO)) { // adult egg
                Message_StartTextbox(this, 0x3066, NULL);
                if (gSaveContext.recoveryItems[SLOT_TRADE_ADULT] == ITEM_POCKET_EGG) gSaveContext.recoveryItems[SLOT_TRADE_ADULT] = ITEM_POCKET_EGG;
            }
    ...
    gSaveContext.moonSpawnFlag = 0;
}

static u16 sChaosJunkItems[] = {
    GI_RUPEE_GREEN_LOSE,
    GI_RECOVERY_HEART,
    GI_DEKU_STICKS_1,
    GI_DEKU_NUTS_5,
...
static u16 sRecoveryGIDs[] = {
    GI_DEKU_STICKS_1, // ITEM_DEKU_STICK,
    GI_DEKU_NUTS_5, // ITEM_DEKU_NUT,
    GI_BOMBS_5, // ITEM_BOMB,
    GI_BOW, // ITEM_BOW,
    ...
    GI_BOTTLE_EMPTY, // ITEM_BOTTLE_MILK_HALF,
    GI_BOTTLE_POE, // ITEM_BOTTLE_POE,
    GI_WEIRD_EGG, // ITEM_WEIRD_EGG,
    GI_CHICKEN, // ITEM_CHICKEN,
    GI_ZELDAS_LETTER, // ITEM_ZELDAS_LETTER,
    GI_MASK_KEATON, // ITEM_MASK_KEATON,
    GI_MASK_SKULL, // ITEM_MASK_SKULL,
    GI_MASK_SPOOKY, // ITEM_MASK_SPOOKY,
    GI_MASK_BUNNY_HOOD, // ITEM_MASK_BUNNY_HOOD,
    GI_MASK_GORON, // ITEM_MASK_GORON,
    GI_MASK_ZORA, // ITEM_MASK_ZORA,
    GI_MASK_GERUDO, // ITEM_MASK_GERUDO,
    GI_MASK_TRUTH, // ITEM_MASK_TRUTH,
    GI_RUPEE_GREEN_LOSE, // ITEM_SOLD_OUT,
    GI_POCKET_EGG, // ITEM_POCKET_EGG,
    GI_POCKET_CUCCO, // ITEM_POCKET_CUCCO,
    GI_COJIRO, // ITEM_COJIRO,
    GI_ODD_MUSHROOM, // ITEM_ODD_MUSHROOM,
    GI_ODD_MUSHROOM, // ITEM_ODD_POTION,
    GI_POACHERS_SAW, // ITEM_POACHERS_SAW,
    GI_BROKEN_GORONS_SWORD, // ITEM_BROKEN_GORONS_SWORD,
    GI_PRESCRIPTION, // ITEM_PRESCRIPTION,
    GI_PRESCRIPTION, // ITEM_EYEBALL_FROG,
    GI_PRESCRIPTION, // ITEM_EYE_DROPS,
    GI_CLAIM_CHECK,// ITEM_CLAIM_CHECK,
};

static u16 sBombBagUpgrades[] = {
    GI_BOMB_BAG_20,
    GI_BOMB_BAG_30,
    GI_BOMB_BAG_40,
};
        ...
        if ((u32)this->transitionMode != TRANS_MODE_OFF) {
            switch (this->transitionMode) {
                case TRANS_MODE_SETUP:
                    if (gSaveContext.finalHoursFlag == 4) {
                        ...
                        if (this->transitionTrigger != TRANS_TRIGGER_END) {
                            ...
                            if ((!(gEntranceTable[this->nextEntranceIndex + sceneLayer].field &
                                ENTRANCE_INFO_CONTINUE_BGM_FLAG)) && !(gSaveContext.finalHoursFlag)) {
                ...
                case TRANS_MODE_INSTANCE_RUNNING:
                    if (this->transitionCtx.isDone(&this->transitionCtx.instanceData)) {
                        if (gSaveContext.finalHoursFlag == 4) { // the world ends
                            gSaveContext.finalHoursFlag = 0;
                            gSaveContext.finalHoursTimer = 0;
                            this->state.running = false;
    ...
    if (1) {
        ...
        if (((R_HREG_MODE != HREG_MODE_PLAY) || R_PLAY_RUN_DRAW)) {
            ...
            if ((R_HREG_MODE != HREG_MODE_PLAY) || R_PLAY_DRAW_COVER_ELEMENTS) {
                ...
                if (this->ageChangeFlag >= 0) {
                    if (this->ageChangeFadeAlpha == 0) {
                        Audio_PlaySfxGeneral(NA_SE_EV_FORM_CHANGE, &gSfxDefaultPos, 4, &gSfxDefaultFreqAndVolScale, &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
                    }
                    this->ageChangeFadeAlpha += TRANSFORM_FADE_SPEED;
                } else {
                    if (this->ageChangeFadeAlpha != 0) {
                        this->ageChangeFadeAlpha -= TRANSFORM_FADE_SPEED;
                    }
                ...
                this->ageChangeFadeAlpha = CLAMP(this->ageChangeFadeAlpha, 0, 255 + TRANSFORM_EXTRA_FADE_FRAMES);
                // draw form change transition
                if (this->ageChangeFadeAlpha != 0) {
                    gDPPipeSync(gfxP++);
                    gSPClearGeometryMode(gfxP++, G_ZBUFFER | G_SHADE | G_CULL_BOTH | G_FOG | G_LIGHTING | G_TEXTURE_GEN |
                                        G_TEXTURE_GEN_LINEAR | G_LOD | G_SHADING_SMOOTH);
                    gDPSetOtherMode(gfxP++, G_AD_DISABLE | G_CD_MAGICSQ | G_CK_NONE | G_TC_FILT | G_TF_BILERP | G_TT_NONE | G_TL_TILE |
                                        G_TD_CLAMP | G_TP_NONE | G_CYC_1CYCLE | G_PM_1PRIMITIVE,
                                    G_AC_NONE | G_ZS_PIXEL | G_RM_CLD_SURF | G_RM_CLD_SURF2);
                    gDPSetCombineLERP(gfxP++, 0, 0, 0, PRIMITIVE, 0, 0, 0, PRIMITIVE, 0, 0, 0, PRIMITIVE, 0, 0, 0, PRIMITIVE);
                    gDPSetPrimColor(gfxP++, 0, 0, 255, 255, 255, CLAMP(this->ageChangeFadeAlpha, 0, 255));
...
void Play_Main(GameState* thisx) {
    ...
    if ((R_HREG_MODE != HREG_MODE_PLAY) || R_PLAY_RUN_UPDATE) {
        if (this->screenshotState != TESTSTATE_SHOW_PICTURE) {
            Play_Update(this);
        }
    }
...
void Play_SpawnScene(PlayState* this, s32 sceneId, s32 spawn) {
    ...
    // special get items for if you're missing some important stuff (I'm so kind aren't I)
    if (LINK_IS_ADULT) {
        ...
        gSaveContext.bButtonMemory = gSaveContext.fierceDeityMemory = gSaveContext.equips.buttonItems[0] = ITEM_SWORD_MASTER;
    }
