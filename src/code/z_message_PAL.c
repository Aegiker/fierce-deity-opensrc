...
u32 sOcarinaSongBitFlags = 0; // ocarina bit flags
...
s16 gOcarinaSongItemMap[] = {
    ...
    OCARINA_SONG_HEALING, // Forest Medallion?
};

s16 gOcarinaTextboxMap[] = {
    0x0893, // Minuet of the Forest
    0x0894, // Bolero of Fire
    0x0895, // Serenade of Water
    0x0896, // Requiem of Spirit
    0x0897, // Nocturne of Shadow
    0x0898, // Prelude of Light
    0x0899, // Saria's Song
    0x089A, // Epona's Song
    0x089B, // Zelda's Lullaby
    0x089C, // Sun's Song
    0x089D, // Song of Time
    0x089E, // Song of Storms
    0x08A0, // Song of Healing
    0x089F, // Scarecrow's Song
};
...
void Message_ResetOcarinaNoteState(void) {
    ...
    sOcarinaButtonAPrimR = 0x50; // n64 colors
...
void Message_UpdateOcarinaMemoryGame(PlayState* play) {
    ...
    if (play->msgCtx.msgMode == MSGMODE_MEMORY_GAME_PLAYER_PLAYING) {
        ...
        AudioOcarina_Start(0x8000, (1 << OCARINA_SONG_MEMORY_GAME)); // Starts the player's turn
        ...
        msgCtx->ocarinaStaff->pos = sOcarinaButtonIndexBufPos = 0;
        AudioOcarina_SetPlaybackSong(OCARINA_SONG_MEMORY_GAME + 1, 1); // Starts the skullkid's turn
        msgCtx->stateTimer = 2;
    }
...
void Message_DrawTextboxIcon(PlayState* play, Gfx** p, s16 x, s16 y) { // n64 colors
...
u32 Message_OffsetItemID(u32 itemId) {
    if (itemId >= ITEM_SWORD_DEITY) { // offset icon by custom item
        itemId += 2;
    }
    return itemId;
}
...
u16 Message_DrawItemIcon(PlayState* play, u16 itemId, Gfx** p, u16 i) {
    ...
    itemId = Message_OffsetItemID(itemId);
...
#define CHECK_FREE_SONG(i) (i == OCARINA_SONG_HEALING) ? 1 : 0
...
void Message_StartOcarinaImpl(PlayState* play, u16 ocarinaActionId) {
    ...
    for (i = sOcarinaSongBitFlags = 0; i <= (OCARINA_SONG_SCARECROW_SPAWN - 1); i++) { 
        if (CHECK_FREE_SONG(i) || CHECK_QUEST_ITEM(QUEST_SONG_MINUET + i)) {
...
void Message_DrawMain(PlayState* play, Gfx** p) {
    static s16 sOcarinaEffectActorIds[] = {
        ...
        ACTOR_OCEFF_WIPE,  ACTOR_OCEFF_STORM, ACTOR_OCEFF_WIPE7, ACTOR_OCEFF_WIPE4,
    };
    ...
    static s16 sOcarinaButtonAPrimColors[][3] = { // n64 colors
        { 0x50, 0x96, 0xFF },
        { 0x64, 0xC8, 0xFF },
    };
    static s16 sOcarinaButtonAEnvColors[][3] = {
        { 10, 10, 10 },
        { 0x32, 0x32, 0xFF }, // n64 colors
    };
    ...
    if (msgCtx->msgLength != 0) {
        ...
        switch (msgCtx->msgMode) {
                ...
                if (msgCtx->msgMode == MSGMODE_OCARINA_STARTING) {
                    if (msgCtx->ocarinaAction == OCARINA_ACTION_UNK_0 ||
                        ...
                        if (msgCtx->ocarinaAction == OCARINA_ACTION_FREE_PLAY ||
                            msgCtx->ocarinaAction == OCARINA_ACTION_CHECK_NOWARP) {
                            AudioOcarina_Start(0xC000, sOcarinaSongBitFlags);
                        } else {
                            // "On Stage Performance"
                            osSyncPrintf("台上演奏\n");
                            AudioOcarina_Start(0, sOcarinaSongBitFlags);
                        }
                    } else {
                        osSyncPrintf("Na_StartOcarinaSinglePlayCheck2( message->ocarina_no );\n");
                        AudioOcarina_Start(0x8000, (1 << msgCtx->ocarinaAction));
                    }
                    ...
                    AudioOcarina_Start(0x8000, (1 << (msgCtx->ocarinaAction - OCARINA_ACTION_PLAYBACK_MINUET)));
                    // "Performance Check"
            ...
            case MSGMODE_OCARINA_PLAYING:
                ...
                if (msgCtx->ocarinaStaff->state < OCARINA_SONG_MEMORY_GAME) {
                    if (msgCtx->ocarinaStaff->state == OCARINA_SONG_SCARECROW_SPAWN ||
                        (CHECK_FREE_SONG(gOcarinaSongItemMap[msgCtx->ocarinaStaff->state]) || CHECK_QUEST_ITEM(QUEST_SONG_MINUET + gOcarinaSongItemMap[msgCtx->ocarinaStaff->state]))) {
            ...
            case MSGMODE_DISPLAY_SONG_PLAYED_TEXT_BEGIN:
                Message_ContinueTextbox(play, gOcarinaTextboxMap[msgCtx->lastPlayedSong]); // You played [song name]
                Message_Decode(play);
