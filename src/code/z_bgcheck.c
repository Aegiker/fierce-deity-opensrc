 ...
 */
s32 BgCheck_TryGetCustomMemsize(s32 sceneId, u32* memSize) { // expand ram
...
Vec3s* BgCheck_GetBgCamFuncDataImpl(CollisionContext* colCtx, s32 bgCamIndex, s32 bgId) {
    ...
    if (SEGMENT_NUMBER(bgCamList[bgCamIndex].bgCamFuncData) > 0x06) { // something went horribly wrong
        return &gMtxClear; // the return value isn't a matrix but that's fine because the pointer leads to zero
    }
...
s32 SurfaceType_GetWallFlags(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId) {
    s32 f = D_80119D90[SurfaceType_GetWallType(colCtx, poly, bgId)];
    return f;
}
