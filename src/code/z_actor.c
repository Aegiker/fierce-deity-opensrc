...
static f32 sFormHeights[NUM_FORMS] = {68.0f, 44.0f, 100.0f}; // Custom Forms, used for camera height and stuff

f32 Player_GetHeight(Player* player) {
    f32 offset = (player->stateFlags1 & PLAYER_STATE1_23) ? 32.0f : 0.0f;

    return offset + sFormHeights[gSaveContext.linkAge];
}
...
void Actor_UpdateAll(PlayState* play, ActorContext* actorCtx) {
    ...
    // spawn dark link or queue loading of object so he can be spawned when it's done
    if (play->darkLinkAmbushFlag != 0) {
        if (Object_IsLoaded(&play->objectCtx, Object_GetIndex(&play->objectCtx, OBJECT_TORCH2))) {
            play->darkLinkAmbushFlag = 0;
            Actor_Spawn(&play->actorCtx, play, ACTOR_EN_TORCH2, player->actor.world.pos.x, player->actor.world.pos.y,
                    player->actor.world.pos.z, 0, 0, 0, 1);
        } else if (play->darkLinkAmbushFlag != 2){
            //Object_Spawn(&play->objectCtx, OBJECT_TORCH2);
            play->darkLinkAmbushFlag = 2;
        }
    }
...
void Actor_Draw(PlayState* play, Actor* actor) {
    ...
    // what if you made everything use GET_PLAYER(play)->draw(actor, play);
