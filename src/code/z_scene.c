...
Gfx* AnimatedMat_TwoLayerTexScroll(PlayState* play, AnimatedMatTexScrollParams* params) {
    return Gfx_TwoTexScroll(play->state.gfxCtx, 0, params[0].xStep * play->gameplayFrames, -(params[0].yStep * play->gameplayFrames),
                            params[0].width, params[0].height, 1, params[1].xStep * play->gameplayFrames,
                            -(params[1].yStep * play->gameplayFrames), params[1].width, params[1].height);
}
...
void AnimatedMat_DrawMain(PlayState* play, AnimatedMaterial* matAnim, f32 alphaRatio, u32 step, u32 flags) {
    static void (*matAnimDrawHandlers[])(PlayState*, s32 segment, void* params) = {
        NULL, AnimatedMat_DrawTwoTexScroll,         NULL,
        NULL, NULL, NULL,
    };
    s32 segmentAbs;
    s32 segment;
...
void Object_InitBank(PlayState* play, ObjectContext* objectCtx) { // expand ram
    PlayState* play2 = play; // Needs to be a new variable to match (possibly a sub struct?)
    u32 spaceSize;
    s32 i;

    if (play2->sceneId == SCENE_HYRULE_FIELD || play2->sceneId == SCENE_LAKE_HYLIA) {
        spaceSize = 1024000;
    ...
    Object_Spawn(&play->objectCtx, OBJECT_TORCH2); // make dark link
    Object_Spawn(&play->objectCtx, OBJECT_RR); // likelike
    switch (play->sceneId) {
...
#define LINK_OBJECT_ALLOC 230000

s32 Object_AllocPlayer(ObjectContext* objectCtx) {
    u32 size;
    s32 playerIndex;

    //Fault_AddHungupAndCrashImpl("Link object was out of memory", "Oopsie!");

    playerIndex = objectCtx->num;
    size = LINK_OBJECT_ALLOC;
...
void Object_LoadPlayer(PlayState* play, s16 age) {
    u32 size;
    s16 objectId = gLinkObjectIds[((void)0, age)];
    ObjectContext* objectCtx = &play->objectCtx;

    gActorOverlayTable[play->playerEntry->id].initInfo->objectId = objectId;
    objectCtx->status[objectCtx->playerIndex].id = objectId;
    size = gObjectTable[objectId].vromEnd - gObjectTable[objectId].vromStart;

    //*((u32*)0x80720000) = objectCtx->status[objectCtx->playerIndex].segment;

    DmaMgr_RequestSyncDebug(objectCtx->status[objectCtx->playerIndex].segment, gObjectTable[objectId].vromStart, size,
                            "../z_scene.c", 145);

    play->ageChangeFlag = -1;
}
...
void Scene_CommandPlayerEntryList(PlayState* play, SceneCmd* cmd) {
    ...
    // we allocating some memory
    play->objectCtx.playerIndex = Object_AllocPlayer(&play->objectCtx);
    play->ageChangeTimer = 0;
    Object_LoadPlayer(play, gSaveContext.linkAge);
}
