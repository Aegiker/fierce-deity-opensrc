...
s32 OnePointCutscene_SetInfo(PlayState* play, s16 subCamId, s16 csId, Actor* actor, s16 timer) {
    ...
    switch (csId) {
        case ONEPOINTDEMO_SMOOTH_RETURN: // smoothly return to main camera from current view
            if (timer < 20) {
                timer = 20;
            }
        ...
        case 5110:
            ...
            break;
        case ONEPOINTDEMO_TRANSFORM_MASK_HUMAN:
            if (Play_CamIsNotFixed(play)) {
                Play_InitCameraDataUsingPlayer(play, subCamId, player, CAM_SET_MASK_TRANSFORMATION0);
            }
            break;
        case ONEPOINTDEMO_TRANSFORM_MASK_FORM:
            if (Play_CamIsNotFixed(play)) {
                Play_InitCameraDataUsingPlayer(play, subCamId, player, CAM_SET_MASK_TRANSFORMATION1);
            }
            break;
        default:
