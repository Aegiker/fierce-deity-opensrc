...
#define DISTORTION_TRANSFORM_1 (1 << 5)
#define DISTORTION_TRANSFORM_2 (1 << 6)
 ...
 */
s32 Camera_Transform0(Camera* camera) {
    Vec3f* eye = &camera->eye;
    Vec3f* at = &camera->at;
    PosRot* focalActorPosRot = &camera->playerPosRot;
    VecGeo atToEye;
    CameraModeValue* values;
    PosRot focalActorFocus;
    f32 sp58;
    f32 sin;
    Transform0ReadOnlyData* roData = &camera->paramData.transform0.roData;
    Transform0ReadWriteData* rwData = &camera->paramData.transform0.rwData;
    s32 pad[2];
    ...
    Actor_GetFocus(&focalActorFocus, camera->player);
    sCameraInterfaceField = roData->interfaceFlags;
    ...
    switch (camera->animState) {
        ...
        case 1:
            ...
            Camera_LERPCeilVec3f(&focalActorFocus.pos, at, 0.2f, 0.2f, 0.1f);
        ...
        case 2:
            ...
            if (rwData->timer == 24) {
                ...
                focalActorFocus.pos.y -= (focalActorFocus.pos.y - camera->playerPosRot.pos.y) * 0.1f;
                focalActorFocus.pos.z = (Math_CosS(focalActorPosRot->rot.y) * -7.0f) + focalActorPosRot->pos.z;
                Camera_LERPCeilVec3f(&focalActorFocus.pos, at, 0.25f, 0.25f, 0.1f);
            }
        ...
        case 4:
            ...
            focalActorFocus.pos.y -= (focalActorFocus.pos.y - camera->playerPosRot.pos.y) * 0.1f;
            Camera_LERPCeilVec3f(&focalActorFocus.pos, at, 0.1f, 0.1f, 0.1f);
    ...
    if (camera->animState == 4) {
        Camera_BGCheck(camera, &camera->at, &camera->eyeNext);
    }
    *eye = camera->eyeNext;
 ...
 */
s32 Camera_Transform1(Camera* camera) {
    Vec3f* eye = &camera->eye;
    Vec3f* at = &camera->at;
    Vec3f* eyeNext = &camera->eyeNext;
    VecGeo atToEye;
    ...
    f32 sp58;
    f32 sin;
    Transform1ReadOnlyData* roData = &camera->paramData.transform1.roData;
    Transform1ReadWriteData* rwData = &camera->paramData.transform1.rwData;
    ...
    switch (camera->animState) {
        case 0:
            ...
            rwData->unk_10 = (Rand_ZeroOne() - 0.5f) * 40.0f;
            camera->roll = 0;
            focalActorFocus.pos.x = camera->playerPosRot.pos.x;
            focalActorFocus.pos.z = camera->playerPosRot.pos.z;
        ...
        case 1:
            ...
            if (rwData->timer <= 0) {
                rwData->timer = 0;
                camera->animState = 2;
                rwData->unk_24 = camera->playerPosRot.rot.y + 0x4000;
                rwData->timer = 46;
                //Distortion_Request(DISTORTION_TYPE_MASK_TRANSFORM_1);
        ...
        case 2:
            ...
            focalActorFocus.pos.x = (Math_SinS(rwData->unk_24) * new_var * rwData->unk_0C) + camera->playerPosRot.pos.x;
            focalActorFocus.pos.z = (Math_CosS(rwData->unk_24) * new_var * rwData->unk_0C) + camera->playerPosRot.pos.z;
            focalActorFocus.pos.y -= (focalActorFocus.pos.y - camera->playerPosRot.pos.y) * 0.2f;
            Camera_LERPCeilVec3f(&focalActorFocus.pos, at, 0.1f, 0.1f, 0.1f);
...
void Camera_UpdateDistortion(Camera* camera) {
    ...
    if (camera->distortionFlags != 0) {
        if (camera->distortionFlags & DISTORTION_UNDERWATER_MEDIUM) {
            ...
            speedFactor = 1.0f;
            scaleFactor = 1.0f;
        } else if (camera->distortionFlags & DISTORTION_TRANSFORM_1) {
            
        } else if (camera->distortionFlags & DISTORTION_TRANSFORM_2) {
...
s16 Camera_ChangeSettingFlags(Camera* camera, s16 setting, s16 flags) {
    ...
    if (((setting == CAM_SET_MEADOW_BIRDS_EYE) || (setting == CAM_SET_MEADOW_UNUSED)) && ERA_IN_FUTURE &&
