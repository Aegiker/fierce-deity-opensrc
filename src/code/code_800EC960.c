 ...
 */
void AudioOcarina_Start(u16 ocarinaFlags, u32 songFlags) {
    u8 i;
    ...
    if (ocarinaFlags != 0xFFFF) {
        sOcarinaFlags = 0x80000000 + (u32)ocarinaFlags; // this is where we're concerned because ocarinaFlags & 0x3FFF is no longer applicable
        sFirstOcarinaSongIndex = 0;
        sLastOcarinaSongIndex = OCARINA_SONG_MAX;
        if ((ocarinaFlags != 0x8000) && (songFlags & (1 << OCARINA_SONG_MEMORY_GAME))) { // Really?! This checked that the flags are (1 << OCARINA_SONG_MEMORY_GAME) + 0x8000!! That's GARBAGE design!!
            sLastOcarinaSongIndex--;
        }
        sAvailOcarinaSongFlags = songFlags;//ocarinaFlags & 0x3FFF;
...
void AudioOcarina_CheckSongsWithoutMusicStaff(void) {
    ...
    if (CHECK_BTN_ANY(sOcarinaInputButtonCur, BTN_L) &&
        ...
        AudioOcarina_Start((u16)sOcarinaFlags, 0);
        return;
    }
