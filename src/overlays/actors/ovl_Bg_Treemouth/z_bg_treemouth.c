...
void BgTreemouth_Init(Actor* thisx, PlayState* play) {
    ...
    if (!IS_CUTSCENE_LAYER && ERA_IN_PAST) {
        BgTreemouth_SetupAction(this, func_808BC8B8);
    } else if (ERA_IN_FUTURE || (gSaveContext.sceneLayer == 7)) {
        this->unk_168 = 0.0f;
...
void func_808BC8B8(BgTreemouth* this, PlayState* play) {
    if (!Flags_GetEventChkInf(EVENTCHKINF_05) || ERA_IN_FUTURE) {
        if (ERA_IN_PAST) {
...
void BgTreemouth_Draw(Actor* thisx, PlayState* play) {
    ...
    if (!IS_CUTSCENE_LAYER || ERA_IN_FUTURE) {
