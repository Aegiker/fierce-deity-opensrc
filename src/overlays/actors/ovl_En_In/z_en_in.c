...
s32 func_80A7975C(EnIn* this, PlayState* play) {
    if (this->actor.params != 1 || this->actor.shape.rot.z != 1 || ERA_IN_PAST) {
        return 0;
    }
    this->animationIdx = 1;
...
s32 func_80A79830(EnIn* this, PlayState* play) {
    if (play->sceneId == SCENE_LON_LON_RANCH && ERA_IN_PAST && IS_DAY && this->actor.shape.rot.z == 1 &&
        !GET_EVENTCHKINF(EVENTCHKINF_TALON_RETURNED_FROM_CASTLE)) {
        return 1;
    }
    if (play->sceneId == SCENE_STABLE && ERA_IN_PAST && IS_DAY && this->actor.shape.rot.z == 3 &&
        GET_EVENTCHKINF(EVENTCHKINF_TALON_RETURNED_FROM_CASTLE)) {
        return 1;
    }
    if (play->sceneId == SCENE_STABLE && ERA_IN_PAST && IS_NIGHT) {
    ...
    if (play->sceneId == SCENE_LON_LON_RANCH && ERA_IN_FUTURE && IS_DAY) {
    ...
    if (play->sceneId == SCENE_LON_LON_BUILDINGS && ERA_IN_FUTURE && IS_NIGHT) {
