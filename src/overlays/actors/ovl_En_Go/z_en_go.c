...
s32 EnGo_IsActorSpawned(EnGo* this, PlayState* play) {
    if (((this->actor.params) & 0xF0) == 0x90) {
        return true;
               ...
               ERA_IN_FUTURE && (this->actor.params & 0xF0) == 0x10) {
        return true;
    } else if (play->sceneId == SCENE_GORON_CITY && ERA_IN_FUTURE && (this->actor.params & 0xF0) == 0x00) {
        return true;
    } else if (play->sceneId == SCENE_DEATH_MOUNTAIN_TRAIL && ERA_IN_PAST &&
               ((this->actor.params & 0xF0) == 0x20 || (this->actor.params & 0xF0) == 0x30 ||
                (this->actor.params & 0xF0) == 0x40)) {
        return true;
    } else if (play->sceneId == SCENE_GORON_CITY && ERA_IN_PAST &&
