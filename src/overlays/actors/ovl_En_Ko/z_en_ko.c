...
f32 func_80A97BC0(EnKo* this) {
    ...
    if (ERA_IN_FUTURE && ENKO_TYPE == ENKO_TYPE_CHILD_FADO) {
        return -20.0f;
    }
...
s32 EnKo_CanSpawn(EnKo* this, PlayState* play) {
    switch (play->sceneId) {
        case SCENE_KOKIRI_FOREST:
            ...
            if (!CHECK_QUEST_ITEM(QUEST_MEDALLION_FOREST) && ERA_IN_FUTURE) {
                return false;
            }
            return true;
        ...
        case SCENE_TWINS_HOUSE:
            if (ERA_IN_FUTURE && !CHECK_QUEST_ITEM(QUEST_MEDALLION_FOREST)) {
