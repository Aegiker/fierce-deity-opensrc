...
u8 BossTw_FindSwordBeam(BossTw* this, PlayState* play, f32 scale) {
    if (this->collider.base.acFlags & AC_HIT) {
        ...
        if (info->toucher.dmgFlags & DMG_SWORD_BEAM) {
    ...
    while (currentSwordbeam != NULL) {
        ...
        if ((fabsf(dx) < scale) && (fabsf(dy) < scale) && (fabsf(dz) < scale)) {
            return true;
        }
...
void BossTw_CheckForSwordbeam(BossTw* this, PlayState* play) {
    ...
    if (this->actionFunc != BossTw_HitByBeam && BossTw_FindSwordBeam(this, play, 60.0f)) {
        ...
        Actor_PlaySfx(&this->actor, NA_SE_EN_TWINROBA_DAMAGE_VOICE);
        play->envCtx.lightBlend = 1.0f;
...
void BossTw_Update(Actor* thisx, PlayState* play) {
    ...
    BossTw_CheckForSwordbeam(this, play);
...
void BossTw_TwinrovaUpdate(Actor* thisx, PlayState* play2) {
    ...
    this->actionFunc(this, play);
    BossTw_TwinrovaCheckForSwordbeam(this, play);
