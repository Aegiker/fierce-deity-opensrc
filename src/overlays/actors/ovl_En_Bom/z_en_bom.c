...
void EnBom_Update(Actor* thisx, PlayState* play2) {
    ...
    if (this->timer == this->timerInit - 3) { // silly
    ...
    if (thisx->params == BOMB_BODY) {
        if (this->timer < (this->timerInit - 7)) {
            dustAccel.y = 0.2f;
        ...
        if ((this->bombCollider.base.acFlags & AC_HIT) || ((this->bombCollider.base.ocFlags1 & OC1_HIT) &&
            ...
            // thanks to chaos this unused feature might actually be useful
        ...
        if ((this->timer < this->timerInit) && ((this->timer & (this->flashSpeedScale + 1)) != 0)) {
