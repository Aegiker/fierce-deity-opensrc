...
void EnSyatekiMan_Destroy(Actor* thisx, PlayState* play) {
    // in case a chaos effect caused you to escape the game before it ended and you decided to be sneaky and leave the building
    if ((gSaveContext.bButtonMemory != ITEM_BOW && gSaveContext.equips.buttonItems[0] == ITEM_BOW) || (gSaveContext.bButtonMemory != ITEM_SLINGSHOT && gSaveContext.equips.buttonItems[0] == ITEM_SLINGSHOT)) {
