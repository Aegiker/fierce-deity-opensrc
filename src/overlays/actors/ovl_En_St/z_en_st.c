...
s32 EnSt_CheckHitFrontside(EnSt* this, PlayState* play) {
    u8 acFlags = this->colCylinder[2].base.acFlags;
    ColliderCylinder* cyl = &this->colCylinder[2];

    if (this->actor.colChkInfo.health == 0) {
        return false;
    }

    if (cyl != NULL && cyl->info.acHitInfo != NULL) {
        s32 flags = (u32)cyl->info.acHitInfo->toucher.dmgFlags;
        if (flags & DMG_SWORD_BEAM) { 
            EnSt_FuckItJustDieInstantly(this, play, flags);
            return false;
        }
    }
...
s32 EnSt_FuckItJustDieInstantly(EnSt* this, PlayState* play, s32 flags) {
    ...
    Actor_SetColorFilter(&this->actor, COLORFILTER_COLORFLAG_RED, 200, COLORFILTER_BUFFLAG_OPA, this->takeDamageSpinTimer);
...
s32 EnSt_CheckColliders(EnSt* this, PlayState* play) {
    if (EnSt_CheckHitFrontside(this, play)) {
