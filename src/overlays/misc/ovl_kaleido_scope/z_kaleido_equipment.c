...
static u8 sEquipmentItemOffsets[] = {
    0x00, ITEM_SWORD_KOKIRI, ITEM_SWORD_MASTER, ITEM_SWORD_BIGGORON,
    0x00, ITEM_SHIELD_DEKU, ITEM_SHIELD_HYLIAN, ITEM_SHIELD_MIRROR,
    0x00, ITEM_TUNIC_KOKIRI, ITEM_TUNIC_GORON, ITEM_TUNIC_ZORA,
    0x00, ITEM_BOOTS_KOKIRI, ITEM_BOOTS_IRON, ITEM_BOOTS_HOVER,
};

static u32 sEquipmentItems[] = {
    ITEM_SWORD_KOKIRI, ITEM_SWORD_MASTER, ITEM_SWORD_BIGGORON,
...
u8 gEquipItems[EQUIP_TYPE_MAX][4] = {
    { 0, ITEM_SWORD_KOKIRI, ITEM_SWORD_MASTER, ITEM_SWORD_BIGGORON },
    { 9, ITEM_SHIELD_DEKU, ITEM_SHIELD_HYLIAN, ITEM_SHIELD_MIRROR },
    { 0, ITEM_TUNIC_KOKIRI, ITEM_TUNIC_GORON, ITEM_TUNIC_ZORA },
    { 9, ITEM_BOOTS_KOKIRI, ITEM_BOOTS_IRON, ITEM_BOOTS_HOVER },
};

u8 KaleidoScope_CheckUsability(u8 y, u8 x) { // i = pauseCtx->cursorY[PAUSE_EQUIP] and j = pauseCtx->cursorX[PAUSE_EQUIP]
    u8 usability = gEquipAgeReqs[y][x];
    u8 item = sEquipmentItemOffsets[x + (y * 4)];

    /*if (LINK_IS_DEITY) {
        return (item == ITEM_TUNIC_KOKIRI || item == ITEM_BOOTS_KOKIRI) || ((ERA_IN_FUTURE) && (item == ITEM_TUNIC_GORON || item == ITEM_TUNIC_ZORA || item == ITEM_BOOTS_IRON || item == ITEM_BOOTS_HOVER));
    }*/

    if (LINK_IS_DEITY) return false;
...
void KaleidoScope_DrawEquipment(PlayState* play) {
    ...
    if ((pauseCtx->state == 6) && (pauseCtx->unk_1E4 == 0) && (pauseCtx->pageIndex == PAUSE_EQUIP)) {
        ...
        if (pauseCtx->cursorX[PAUSE_EQUIP] == 0) {
            ...
            cursorItem = sEquipmentItemOffsets[pauseCtx->cursorPoint[PAUSE_EQUIP]];
            osSyncPrintf("ccc=%d\n", cursorItem);
        ...
        if (!KaleidoScope_CheckUsability(pauseCtx->cursorY[PAUSE_EQUIP], pauseCtx->cursorX[PAUSE_EQUIP])) {
    ...
    for (rowStart = 0, i = 0, point = 4; i < 4; i++, rowStart += 4, point += 16) {
        ...
        for (k = 0, temp = rowStart + 1, bit = rowStart, j = point; k < 3; k++, bit++, j += 4, temp++) {
            ...
            if ((gBitFlags[bit] & gSaveContext.inventory.equipment) && (pauseCtx->cursorSpecialPos == 0)) {
                if (KaleidoScope_CheckUsability(i, k + 1)) {
                    if (temp == cursorSlot) {
    ...
    for (rowStart = 0, j = 0, temp = 0, i = 0; i < 4; i++, rowStart += 4, j += 16) {
        ...
        for (k = 0, bit = rowStart, point = 4; k < 3; k++, point += 4, temp++, bit++) {
            ...
            if (((u32)i == 0) && (k == 2) && (gSaveContext.bgsFlag != 0)) {
                ...
                KaleidoScope_DrawQuadTextureRGBA32(play->state.gfxCtx, gItemIcons[sEquipmentItems[temp]],
